...
function confirmExit() {
	return document.getElementById('confirmBreak').innerText;
}
// Attach event handler
	...
	req.onreadystatechange = function() {
		...
		if (req.readyState == XMLHttpRequest.DONE) {
			...
			if (req.statusText == 'OK') {
				statusElem.innerHTML = "<span data-icon='ok'></span>"
			} else {
				statusElem.innerHTML = "<span data-icon='delete'>" +
					req.statusText + "</span>" // show status (Not Found, ...)
			}
...
function improveAction() {
	...
	if (improveType.value == '')
		...
		improveText.value = document.getElementById(improveType.value).innerText;
}
...
function statusbar(status, ticker) {
	...
	if (status == '') {
		...
		if (ticker != '') {
			as.innerHTML = '<img src="/styles/default/images/loader.gif"/>';
		}
	}
}
